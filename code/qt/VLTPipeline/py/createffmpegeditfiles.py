...
def CreateFFMPEGConcat(RenderIDSJson, exportdirectory, ProjectName, audioFile, Projectt):
    eeveefiles = []
    eeveefilepath = exportdirectory + "/pipeline/ffmpegconcat_eevee.txt"
    eeveeconcactbatch = "ffmpeg -f concat -safe 0 -i \"" +eeveefilepath + "\" -c copy \"" + exportdirectory +"/" +Projectt.Version +"/"+ ProjectName+ "_eevee_edit"+date_time+"nosound.mp4\" -y"
    eeveeconcactbatchsound = "ffmpeg -i \"" + exportdirectory +"/" +Projectt.Version +"/" + ProjectName+ "_eevee_edit"+date_time+"nosound.mp4\""+" -i \"" +audioFile+   "\" -map 0:v -map 1:a -c:v copy -shortest \"" + exportdirectory +"/" + ProjectName+ "_eevee_edit"+date_time+"sound.mp4\" -y"
    eeveeconcactbatchfilepath = exportdirectory + "/pipeline/ffmpegconcat_eevee.bat"
    print("0 | " , RenderIDSJson["Clips"])
    
    for f in RenderIDSJson["Clips"]:
        print("1 | " , RenderIDSJson["Clips"])
        print("2 | " , f["VidFileEevee"])
    ...
    pbconcactbatch = "ffmpeg -f concat -safe 0 -i \"" +pbfilepath + "\" -c copy \"" + exportdirectory +"/"+Projectt.Version +"/" + ProjectName+ "_pb_edit"+date_time+"nosound.mp4\" -y"
    pbconcactbatchsound = "ffmpeg -i \"" + exportdirectory +"/"+Projectt.Version +"/" + ProjectName+ "_pb_edit"+date_time+"nosound.mp4\""+" -i \"" +audioFile+   "\" -map 0:v -map 1:a -c:v copy -shortest \"" + exportdirectory +"/" + ProjectName+ "_pb_edit"+date_time+"sound.mp4\" -y"
...
def CreateFFMPEGConcatRun(RenderIDSJson, exportdirectory, ProjectName, audioFile, Projectt):
    eeveefiles = []
    eeveefilepath = exportdirectory + "/pipeline/ffmpegconcat_eevee.txt"
    eeveeconcactbatch = "ffmpeg -f concat -safe 0 -i \"" +eeveefilepath + "\" -c copy \"" + exportdirectory +"/"+Projectt.Version +"/" + ProjectName+ "_eevee_edit"+date_time+"nosound.mp4\" -y"
    eeveeconcactbatchsound = "ffmpeg -i \"" + exportdirectory +"/" +Projectt.Version +"/"+ ProjectName+ "_eevee_edit"+date_time+"nosound.mp4\""+" -i \"" +audioFile+   "\" -map 0:v -map 1:a -c:v copy -shortest \"" + exportdirectory +"/" + ProjectName+ "_eevee_edit"+date_time+"sound.mp4\" -y"
    ...
    if not os.path.exists(exportdirectory +"/"+Projectt.Version):
        os.makedirs(exportdirectory +"/"+Projectt.Version)
    ...
    subprocess.call([eeveeconcactbatchfilepath])
    subprocess.call([pbconcactbatchfilepath])

    print("Saved to : ", eeveefilepath)
    

def RunMainProcess(editjsonpath, exportdirectory, ProjectName, audioFile, Projectt):
    outfolder = exportdirectory + "/" + Projectt.Version
    editjson = LoadJSON(editjsonpath)
    foldersarray = GetDirectories(outfolder)
    foldernamesarray = CleanNames(foldersarray)
    editjsonarray = CleanEditShotIDS(editjson)
    RenderIdsJSON = GetRenderIDS(editjsonarray, foldernamesarray, outfolder)
    CreateFFMPEGConcat(RenderIdsJSON, exportdirectory,ProjectName, audioFile,Projectt)
    print(RenderIdsJSON)

def RunMainProcessBats(editjsonpath, exportdirectory, ProjectName, audioFile,Projectt):
    outfolder = exportdirectory + "/" + Projectt.Version
    ...
    editjsonarray = CleanEditShotIDS(editjson)
    RenderIdsJSON = GetRenderIDS(editjsonarray, foldernamesarray, outfolder)
    CreateFFMPEGConcatRun(RenderIdsJSON, exportdirectory,ProjectName, audioFile,Projectt)
    print(RenderIdsJSON)

def RunProcessProject(Projectt):
    print("Audiofile ", os.path.exists(Projectt.AudioFile))
    print("OutputFolder ", os.path.exists(Projectt.OutputFolder))
    print("EditJson ", os.path.exists(Projectt.EditJson))
    print("EditJson ",Projectt.EditJson)
    if os.path.exists(Projectt.AudioFile) &  os.path.exists(Projectt.OutputFolder) &  os.path.exists(Projectt.EditJson): 
        RunMainProcess(Projectt.EditJson, Projectt.OutputFolder, Projectt.ProjectName, Projectt.AudioFile, Projectt )
        return True
    else:
        return False
    
def RunProcessBatsProject(Projectt):
    if os.path.exists(Projectt.AudioFile) &  os.path.exists(Projectt.OutputFolder) &  os.path.exists(Projectt.EditJson): 
        RunMainProcessBats(Projectt.EditJson, Projectt.OutputFolder, Projectt.ProjectName, Projectt.AudioFile, Projectt )
        return True
    else:
        return False
