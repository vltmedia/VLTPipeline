...
class Project:
    ...
    def FromIni(self,filepath):
        if os.path.exists(filepath):
            ...
            try:
                my_list = ast.literal_eval(config.get("blender", "blendfiles"))

                self.BlendFiles = my_list
                # self.BlendFiles = config['blender']['blendfiles']
            except :
                self.BlendFiles = []
            return True
        else:
    ...
    def CreateConfig(self, OutputPath):
        ...
        if not os.path.exists(self.OutputFolder + "/pipeline"):
            os.makedirs(self.OutputFolder + "/pipeline")
